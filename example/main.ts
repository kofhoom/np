...
viewer.initialize(targetEl, DEV_MODE).then(() => {
  ...
  if (IS_MOBILE) {
    (sky.material as any).vertexShader = [
      ...
      'float sunIntensity(float zAC){zAC=clamp(zAC,-1.0,1.0);return EE*max(0.0,1.0-exp(-((cutoffAngle-acos(zAC))/steepness)));}',
      'void main(){',
      ...
      '  vec3 mieK=vec3(0.686,0.678,0.666);',
      '  float v2=4.0-2.0;',
      '  vec3 lam=vec3(680.0e-9,550.0e-9,450.0e-9);',
      '  vec3 mieCalc=pi*pow((2.0*pi)/lam,vec3(v2))*mieK;',
      '  vBetaM=0.434*c*mieCalc*mieCoefficient;',
      '}',
    ].join('\n');
