...
saveBtn.style.cssText = 'flex:1;background:#226622;color:#fff;border:none;padding:6px;border-radius:4px;cursor:pointer;font-size:12px;';
const coordBtn = document.createElement('button');
coordBtn.textContent = '📍 좌표 확인';
coordBtn.style.cssText = 'flex:1;background:#554400;color:#fff;border:none;padding:6px;border-radius:4px;cursor:pointer;font-size:12px;';
coordBtn.addEventListener('click', () => {
  const p = viewer.camera.position;
  const t = viewer.cameraControls.target;
  alert(`카메라: (${p.x.toFixed(3)}, ${p.y.toFixed(3)}, ${p.z.toFixed(3)})\n타겟: (${t.x.toFixed(3)}, ${t.y.toFixed(3)}, ${t.z.toFixed(3)})`);
});
const hideBtn = document.createElement('button');
hideBtn.textContent = '패널 숨기기';
hideBtn.style.cssText = 'flex:1;background:#444;color:#fff;border:none;padding:6px;border-radius:4px;cursor:pointer;font-size:12px;';
topRow.appendChild(saveBtn); topRow.appendChild(coordBtn); topRow.appendChild(hideBtn);
...
function makeSlider(label: string, min: number, max: number, value: number, onChange: (v: number) => void): void {
  const wrap = document.createElement('div');
  wrap.style.cssText = 'display:flex;align-items:center;gap:6px;margin:3px 0;';
  const lbl = document.createElement('span');
  lbl.textContent = label; lbl.style.cssText = 'width:115px;font-size:11px;color:#ccc;flex-shrink:0;';
  const input = document.createElement('input');
  input.type = 'range'; input.min = String(min); input.max = String(max);
  input.step = String((max - min) / 200); input.value = String(value); input.style.cssText = 'flex:1;';
  const valEl = document.createElement('span');
  valEl.textContent = value.toFixed(2); valEl.style.cssText = 'width:38px;font-size:11px;color:#aaa;text-align:right;';
  input.addEventListener('input', () => { const v = parseFloat(input.value); valEl.textContent = v.toFixed(2); onChange(v); });
  wrap.appendChild(lbl); wrap.appendChild(input); wrap.appendChild(valEl);
  panel.appendChild(wrap);
}

function sep(label: string): void {
  const d = document.createElement('div');
  d.style.cssText = 'color:#666;font-size:11px;margin:8px 0 3px;border-top:1px solid #333;padding-top:5px;';
  d.textContent = `── ${label} ──`;
  panel.appendChild(d);
}
...
loader.load(
  ...
  (gltf) => {
    ...
    model.traverse((obj: any) => { if (obj.isMesh && obj.geometry) obj.geometry.computeBoundsTree(); });
    acousticModel = model;
    if (saved.model) {
    ...
    modelTC.addEventListener('dragging-changed', (e: any) => { viewer.cameraControls.enabled = !e.value; });
    ...
    (['이동', '회전', '크기', '숨기기'] as const).forEach((txt, i) => {
      ...
      btn.addEventListener('click', () => {
        if (i === 3) { modelTC.visible = false; }
        else { modelTC.visible = true; modelTC.setMode(['translate', 'rotate', 'scale'][i] as any); }
        modelBtnRow.querySelectorAll('button').forEach((b, j) => { (b as HTMLElement).style.background = j === i ? '#2255aa' : '#444'; });
      });
    ...
    sep('전역 설정');
    makeSlider('환경광', 0, 2, saved.ambientIntensity ?? 1.66, (v) => { ambient.intensity = v; });
    makeSlider('노출', 0.2, 3, saved.exposure ?? 0.284, (v) => { viewer.renderer.toneMappingExposure = v; });

    sep('☀️ 태양 위치');
    let curAz = sunAzimuth, curEl = sunElevation;
    makeSlider('방위각 (좌우)', 0, 360, sunAzimuth, (v) => { curAz = v; updateSun(curAz, curEl); });
    makeSlider('고도 (상하)', -5, 30, sunElevation, (v) => { curEl = v; updateSun(curAz, curEl); });
    makeSlider('태양 빛 강도', 0, 10, saved.sunLightIntensity ?? 3.15, (v) => { sunLight.intensity = v; });

    saveBtn.addEventListener('click', () => {
      saveSettings({
        ...loadSettings(),
        model: { px: model.position.x, py: model.position.y, pz: model.position.z, rx: model.rotation.x, ry: model.rotation.y, rz: model.rotation.z, sx: model.scale.x, sy: model.scale.y, sz: model.scale.z },
        camera: { px: viewer.camera.position.x, py: viewer.camera.position.y, pz: viewer.camera.position.z, tx: viewer.cameraControls.target.x, ty: viewer.cameraControls.target.y, tz: viewer.cameraControls.target.z },
        ambientIntensity: ambient.intensity,
        exposure: viewer.renderer.toneMappingExposure,
        sunAzimuth: curAz, sunElevation: curEl, sunLightIntensity: sunLight.intensity,
        emitters: emitters.map(em => ({ name: em.name, x: em.x, y: em.y, z: em.z, params: { ...em.params } })),
      });
      saveBtn.textContent = '✅ 저장됨';
      setTimeout(() => { saveBtn.textContent = '💾 설정 저장'; }, 1500);
    });
    ...
    targetEl.addEventListener('dblclick', (e) => {
      ...
      if (hits.length > 0) {
        ...
        const dx2 = viewer.camera.position.x - p.x, dy2 = viewer.camera.position.y - p.y, dz2 = viewer.camera.position.z - p.z;
    ...
    const dx = viewer.camera.position.x - initTarget.x, dy = viewer.camera.position.y - initTarget.y, dz = viewer.camera.position.z - initTarget.z;
  ...
  (xhr) => { if (xhr.total > 0) loadingEl.textContent = `로딩 중... ${Math.round((xhr.loaded / xhr.total) * 100)}%`; },
...
const REFLECT_DIRS = [
  ...
  new Vector3(_s, _s, _s), new Vector3(-_s, _s, _s),
  new Vector3(_s, -_s, _s), new Vector3(-_s, -_s, _s),
  new Vector3(_s, _s, -_s), new Vector3(-_s, _s, -_s),
  new Vector3(_s, -_s, -_s), new Vector3(-_s, -_s, -_s),
];
...
function createEmitterNodes(em: AudioEmitter): void {
  ...
  const p = em.params;
  em.gainNode = audioCtx.createGain(); em.gainNode.gain.value = p.volume;
  em.panner = audioCtx.createPanner();
  em.panner.panningModel = 'HRTF'; em.panner.distanceModel = p.distanceModel;
  em.panner.refDistance = p.refDistance; em.panner.rolloffFactor = p.rolloffFactor; em.panner.maxDistance = p.maxDistance;
  ...
  em.preDelayNode = audioCtx.createDelay(0.5); em.preDelayNode.delayTime.value = p.preDelay;
...
(function () {
  ...
  if (savedEmitters && savedEmitters.length > 0) {
    ...
    emitters.push(createEmitter(
      saved.audioSrc?.x ?? 7.1655, saved.audioSrc?.y ?? 25.24, saved.audioSrc?.z ?? -4.4519,
      'emitter 1', saved.audioParams
    ));
  }
})();
...
prevBtn.textContent = 'prev'; prevBtn.style.cssText = 'cursor:pointer;opacity:0.7;';
...
nextBtn.textContent = 'next'; nextBtn.style.cssText = 'cursor:pointer;opacity:0.7;';

const spatialBtn = document.createElement('span');
spatialBtn.textContent = '360'; spatialBtn.title = '360 공간음향 on/off';
...
srcToggleBtn.textContent = '🔊'; srcToggleBtn.title = DEV_MODE ? '음원 위치 조정' : '볼륨';
srcToggleBtn.style.cssText = 'cursor:pointer;opacity:0.7;font-size:13px;';

playerEl.appendChild(prevBtn); playerEl.appendChild(trackNameEl);
playerEl.appendChild(nextBtn); playerEl.appendChild(spatialBtn); playerEl.appendChild(srcToggleBtn);
...
volLabel.textContent = '볼륨'; volLabel.style.cssText = 'color:#aaa;font-size:10px;';
...
volKnobInput.type = 'range'; volKnobInput.min = '0'; volKnobInput.max = '2'; volKnobInput.step = '0.01';
...
srcTitle.textContent = '음원 위치'; srcTitle.style.cssText = 'color:#aaa;font-size:10px;margin-bottom:6px;';
srcPanel.appendChild(srcTitle);

function makeSrcSlider(label: string, min: number, max: number, initVal: number, onChange: (v: number) => void): void {
  const row = document.createElement('div');
  row.style.cssText = 'display:flex;align-items:center;gap:6px;margin:3px 0;';
  const lbl = document.createElement('span');
  lbl.textContent = label; lbl.style.cssText = 'width:14px;color:#ccc;font-size:11px;flex-shrink:0;';
  const inp = document.createElement('input');
  inp.type = 'range'; inp.min = String(min); inp.max = String(max);
  inp.step = String((max - min) / 400); inp.value = String(initVal); inp.style.cssText = 'flex:1;';
  const val = document.createElement('span');
  val.textContent = initVal.toFixed(2); val.style.cssText = 'width:42px;font-size:11px;color:#aaa;text-align:right;';
  inp.addEventListener('input', () => {
    const v = parseFloat(inp.value); val.textContent = v.toFixed(2);
    onChange(v); applyEmitterPos(activeEm());
  });
...
function makeAudioSlider(label: string, min: number, max: number, initVal: number, onChange: (v: number) => void): void {
  const row = document.createElement('div');
  row.style.cssText = 'display:flex;align-items:center;gap:6px;margin:3px 0;';
  const lbl = document.createElement('span');
  lbl.textContent = label; lbl.style.cssText = 'width:90px;color:#ccc;font-size:11px;flex-shrink:0;';
  const inp = document.createElement('input');
  inp.type = 'range'; inp.min = String(min); inp.max = String(max);
  inp.step = String((max - min) / 400); inp.value = String(initVal); inp.style.cssText = 'flex:1;';
  const val = document.createElement('span');
  ...
  inp.addEventListener('input', () => {
    const v = parseFloat(inp.value); val.textContent = v.toFixed(2);
    onChange(v); applyEmitterParams(activeEm());
  });
  row.appendChild(lbl); row.appendChild(inp); row.appendChild(val);
  srcPanel.appendChild(row);
}
...
const em0 = activeEm();
makeSrcSlider('X', -50, 50, em0.x, (v) => { activeEm().x = v; });
makeSrcSlider('Y', -50, 50, em0.y, (v) => { activeEm().y = v; });
makeSrcSlider('Z', -50, 50, em0.z, (v) => { activeEm().z = v; });

makeSep2('볼륨 / 거리');
makeAudioSlider('볼륨', 0, 2, em0.params.volume, (v) => { activeEm().params.volume = v; });
makeAudioSlider('기준 거리', 0.1, 30, em0.params.refDistance, (v) => { activeEm().params.refDistance = v; });
makeAudioSlider('감쇠 계수', 0, 5, em0.params.rolloffFactor, (v) => { activeEm().params.rolloffFactor = v; });
makeAudioSlider('최대 거리', 10, 500, em0.params.maxDistance, (v) => { activeEm().params.maxDistance = v; });
...
distModelLbl.textContent = '감쇠 방식'; distModelLbl.style.cssText = 'width:90px;color:#ccc;font-size:11px;flex-shrink:0;';
const distModelSel = document.createElement('select');
distModelSel.style.cssText = 'flex:1;background:#222;color:#ccc;border:1px solid #444;border-radius:4px;font-size:11px;padding:2px;';
...
distModelRow.appendChild(distModelLbl); distModelRow.appendChild(distModelSel);
srcPanel.appendChild(distModelRow);

makeSep2('리버브 (잔향)');
makeAudioSlider('Wet (잔향량)', 0, 1, em0.params.reverbWet, (v) => { activeEm().params.reverbWet = v; });
makeAudioSlider('잔향 길이(초)', 0.1, 10, em0.params.reverbDuration, (v) => { activeEm().params.reverbDuration = v; });
makeAudioSlider('감쇠 곡선', 0.1, 8, em0.params.reverbDecay, (v) => { activeEm().params.reverbDecay = v; });
makeAudioSlider('조기 반사음', 0, 1.5, em0.params.earlyReflections, (v) => { activeEm().params.earlyReflections = v; });
makeAudioSlider('프리딜레이(초)', 0, 0.2, em0.params.preDelay, (v) => { activeEm().params.preDelay = v; });
makeAudioSlider('스테레오 폭', 0, 1, em0.params.stereoWidth, (v) => { activeEm().params.stereoWidth = v; });
...
autoRoomLabel.textContent = '공간 기반 잔향 자동 조절'; autoRoomLabel.style.cssText = 'color:#ccc;font-size:11px;';
autoRow.appendChild(autoToggle); autoRow.appendChild(autoRoomLabel);
...
makeAudioSlider('반사 강도', 0, 2, em0.params.earlyReflections, (v) => { activeEm().params.earlyReflections = v; });
...
snapBtn.addEventListener('click', () => {
  const em = activeEm();
  em.x = viewer.camera.position.x; em.y = viewer.camera.position.y; em.z = viewer.camera.position.z;
  applyEmitterPos(em);
  srcPanel.querySelectorAll<HTMLInputElement>('input[type=range]').forEach((inp, i) => {
    if (i > 2) return;
    const vals = [em.x, em.y, em.z];
    inp.value = String(vals[i]); (inp.nextSibling as HTMLElement).textContent = vals[i].toFixed(2);
  });
});
...
stepLbl.textContent = '이동 단위'; stepLbl.style.cssText = 'width:90px;color:#ccc;font-size:11px;flex-shrink:0;';
...
function moveSrc(axis: 'forward' | 'right' | 'up', sign: number): void {
  ...
  const fwd = new Vector3(); viewer.camera.getWorldDirection(fwd); fwd.y = 0; fwd.normalize();
  const right = new Vector3(); right.crossVectors(fwd, new Vector3(0, 1, 0)).normalize();
  const dir = new Vector3();
  if (axis === 'forward') dir.copy(fwd);
  else if (axis === 'right') dir.copy(right);
  else dir.set(0, 1, 0);
  const em = activeEm();
  em.x += dir.x * step * sign; em.y += dir.y * step * sign; em.z += dir.z * step * sign;
  applyEmitterPos(em); refreshXYZSliders();
}
...
setInterval(() => {
  ...
  const fwd = new Vector3(); viewer.camera.getWorldDirection(fwd);
      ...
      for (const em of emitters) {
        ...
        if (em.reflDelays.length === REFLECT_DIRS.length) {
          for (let i = 0; i < REFLECT_DIRS.length; i++) {
            ...
            const reflGain = wallDist < 100
              ? Math.min(0.35, em.params.earlyReflections * (1 - absorption) / (wallDist * 0.5 + 1))
              : 0;
          ...
          if (!bothInside) {
            ...
            if (em.lastAutoDuration === 0 && em.convolver) {
              ...
              em.convolver.buffer = makeImpulseResponse(
                audioCtx, em.lastAutoDuration, em.params.reverbDecay, autoStereoWidth
              );
            }
